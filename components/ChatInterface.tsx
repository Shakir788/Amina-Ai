...
// ... (Avatar, ImageGenerator, InvoiceTable, YouTubePlayer, StopAction, RenderToolInvocation same as before)
// (Code size reduction ke liye unhe repeat nahi kar raha, wo purane wale hi rahenge)
...
export default function ChatInterface() {
  ...
  const speakChunk = async (text: string, lang: string) => {
      ...
      try {
          const res = await fetch("/api/speak", { 
              ...
              body: JSON.stringify({ text, voice: voiceGender, lang }),
              signal
          });
          
          if (!res.ok) throw new Error("TTS Failed");
          
          // 🚨 CRITICAL CHECK: After fetch
          if (isInterruptedRef.current) return;
          ...
          if (e.name !== 'AbortError') console.error("Chunk Error:", e);
      }
  };
  ...
  const speak = async (rawText: string, messageId: string) => {
    ...
    // 🔥 KILL PREVIOUS LOOP IMMEDIATELY
    isInterruptedRef.current = true; // Signal kill
    stopSpeaking(); 
    
    // Wait a tick to ensure clean state
    await new Promise(r => setTimeout(r, 50));
    isInterruptedRef.current = false; // Ready for new
  ...
  const startListening = () => {
    ...
    recognition.onresult = (e: any) => { 
        ...
        // 🔥 KILL SWITCH TRIGGER
        if (isSpeaking && t && t.trim().length > 2) {
            console.log("BARGE-IN DETECTED: " + t);
            isInterruptedRef.current = true; // Raise Flag
            stopSpeaking(); 
    ...
    recognition.onend = () => { 
        if (isCallActive && !isLoading) {
             setTimeout(() => { if(isCallActive) startListening(); }, 100);
        }
    };
